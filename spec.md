...
HTML sessions are admitted for readable `.html`/`.htm` files and have `documentKind: "html"`; absent `documentKind` in legacy sidecars remains `markdown`. Session creation resolves the source path once and captures one immutable source string plus SHA-256. The artifact index injects one deterministic SDK tag into the HTTP response only; canonical HTML is never rewritten.

The trusted shell embeds the source snapshot in an opaque-origin `sandbox="allow-scripts"` iframe. The artifact CSP is explicit: `default-src 'none'`, self/inline scripts and styles, same-root/data images and fonts, and no connections, objects, media, frames/children, workers, manifests, forms, base URI, or navigation. Relative assets are decoded once and must be regular files under the source parent after both lexical and resolved-realpath confinement, with an explicit MIME allowlist, `no-store`, and `nosniff`; the opened descriptor is rechecked after confinement to close symlink races. The shell remains the only token holder. Artifact messages require the iframe WindowProxy, correlation, a parent-issued per-load handshake challenge, and per-load capability; opaque `event.origin` is deliberately not used.

HTML operations never overload Markdown `range`. `html-element` requires a unique selector, exact tag, document-root child path, bounded normalized text, and visual rectangle. `html-text-range` additionally stores a common ancestor, each boundary selector/path/offset, bounded normalized and raw selection evidence, and visual rectangle. `mermaid-node` stores diagram ID, node ID, label, selector, and rectangle. Server parsing is total and bounded (including a 16-compound selector-depth limit); malformed targets, ranges, operation payloads, and scope/type combinations return `400` before sidecar mutation. The SDK reconstructs identity from all anchors and restores precise ranges; geometry is evidence only, with signed viewport coordinates preserved. Missing, changed, ambiguous, delayed, shadow-DOM, and cross-origin targets remain visibly **Unresolved target** and never fall back to similarity.

Mermaid source blocks are retained as a selectable fallback and a narrow dependency-free renderer may expose rendered nodes for `mermaid-node` operations. This does not add diagram editing, Excalidraw, scene persistence, or source conversion.

If disk bytes drift, the shell keeps displaying the captured snapshot and shows stale status; operation mutation and Finish return `409` while retaining the sidecar. Confirmed Reload intentionally clears open operations, adopts a fresh snapshot, and rotates the frame capability. HTML Finish atomically writes `<source-basename>-commented.json` only when open operations exist. The envelope contains `schemaVersion`, source path, `documentKind`, source hash, creation metadata, and open structured operations; document notes are JSON operations. No-open Finish and Abort remove temporary state and emit no handoff. The canonical HTML bytes remain unchanged. `npm run browser:test` is the maintained Playwright browser validation command and must record external fixture hash/size before and after without copying the fixture.

Residual limitations are explicit: shadow-DOM content and cross-origin iframe contents are outside the target boundary; source rewrites that fail evidence stay unresolved; forms, popups, downloads, top-level/nested navigation, remote active content, and privileged filesystem/API access are intentionally blocked.

Revision log: 2026-09-14 — expanded the HTML artifact contract with immutable snapshots, strict target identity, Mermaid fallback/node support, CSP/asset confinement, protocol capabilities, unresolved accessibility, lifecycle, and browser evidence.
