...
The trusted shell embeds the source snapshot in an opaque-origin `sandbox="allow-scripts"` iframe. The artifact CSP is explicit: `default-src 'none'`, self/inline scripts and styles, same-root/data images and fonts, and no connections, objects, media, frames/children, workers, manifests, forms, base URI, or navigation. Relative assets are decoded once and must be regular files under the source parent after both lexical and resolved-realpath confinement, with an explicit MIME allowlist, `no-store`, and `nosniff`. The shell remains the only token holder. Artifact messages require the iframe WindowProxy, correlation, and per-load capability; opaque `event.origin` is deliberately not used.

HTML operations never overload Markdown `range`. `html-element` requires a unique selector, exact tag, document-root child path, bounded normalized text, and visual rectangle. `html-text-range` additionally stores a common ancestor, each boundary selector/path/offset, bounded normalized and raw selection evidence, and visual rectangle. `mermaid-node` stores diagram ID, node ID, label, selector, and rectangle. Server parsing is total and bounded; malformed targets, ranges, operation payloads, and scope/type combinations return `400` before sidecar mutation. The SDK reconstructs identity from all anchors and restores precise ranges; geometry is evidence only. Missing, changed, ambiguous, delayed, shadow-DOM, and cross-origin targets remain visibly **Unresolved target** and never fall back to similarity.
