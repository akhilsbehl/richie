...
test("accepts every target kind and rejects malformed, ambiguous, or unbounded evidence", () => {
  ...
  const deepSelector = Array.from({ length: HTML_LIMITS.selectorDepth + 1 }, () => "div").join(" > ");
  const invalid = [null, [], {}, { type: "unknown", selector: "#x", rect },
    { type: "html-element", selector: ["div", "span"].join(","), path: [], tag: "p", text: "x", rect },
    { type: "html-element", selector: "   ", path: [], tag: "p", text: "x", rect },
    { type: "html-element", selector: "!!!", path: [], tag: "p", text: "x", rect },
    { type: "html-element", selector: deepSelector, path: [], tag: "p", text: "x", rect },
    ...
    { type: "html-text-range", selector: "#other", commonAncestorSelector: "#x", start: boundary, end: { ...boundary, offset: 4 }, text: "x", exactText: "x", rect },
...
test("rectangle validation allows signed viewport coordinates but requires finite dimensions and usable viewport", () => {
  assert.equal(isValidHtmlRect(rect), true);
  assert.equal(isValidHtmlRect({ ...rect, viewport: { ...point, x: -12, y: -24 } }), true);
